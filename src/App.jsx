...
export default function SkillTreeBuilderDnd() {
  ...
  const topToggleBtnRef = useRef(null); // botão que abre/fecha o menu superior

  // Painel lateral via BOTÃO (igual ao superior) + fechar ao clicar fora
  const [showRightPanel, setShowRightPanel] = useState(false);
  const rightPanelRef = useRef(null);
  const rightToggleBtnRef = useRef(null); // botão que abre/fecha o painel lateral

  const [showMiniMap, setShowMiniMap] = useState(true);

  // Fechar painel superior ao clicar fora, mas IGNORAR cliques no botão de toggle
  useEffect(() => {
    const onDown = (e) => {
  ...
  // Fechar painel lateral ao clicar fora, mas IGNORAR cliques no botão de toggle
  useEffect(() => {
    const onDown = (e) => {
  ...
  const isValidConnection = useCallback((conn) => {
    ...
    // Verifica se ao adicionar a conexão, formaria um ciclo
  ...
  return (
    <div className={cx("w-full h-screen flex", isDark ? "bg-zinc-900 text-zinc-100" : "bg-slate-50 text-slate-900")}>
      <div className="flex-1 relative">
        ...
        {/* Painel direito agora controlado por botão (sem hover) */}
        <aside
          ref={rightPanelRef}
